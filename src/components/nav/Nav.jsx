...
const Navbar = () => {
    ...
    return (
        <nav className='fixed top-0 left-0 w-full h-20 flex items-center justify-between md:px-16 lg:px-24 xl:px-32 bg-white/10 z-50'>
        ...
        <div className={`max-md:fixed max-md:inset-0 max-md:bg-white/50 max-md:overflow-hidden max-md:transition-[width] max-md:duration-300 max-md:top-0 max-md:left-0 max-md:flex-col max-md:justify-center max-md:text-lg max-md:backdrop-blur flex items-center gap-8 ${ menuOpen ? 'max-md:w-full' : 'max-md:w-0' }`} >
            ...
            <div className="mt-5 nav-status-container shadow-lg flex items-center justify-around gap-6 px-6">
                {/* Home Link */}
                <Link to="/#home" className="relative overflow-hidden h-6 group text-white text-sm mx-2" onClick={() => setMenuOpen(false)}>
                ...
                {/* About Link */}
                <Link smooth to="/#about" className="relative overflow-hidden h-6 group text-white text-sm mx-2" onClick={() => setMenuOpen(false)}>
                        <span className="block group-hover:-translate-y-full transition-transform duration-300">
                            About
                        </span>
                        <span className="block absolute top-full left-0 group-hover:translate-y-[-100%] transition-transform duration-300 text-emerald-400">
                ...
                {/* Contact Link */}
                <Link to="/#Contact" className="relative overflow-hidden h-6 group text-white text-sm mx-2" onClick={() => setMenuOpen(false)}>
                        <span className="block group-hover:-translate-y-full transition-transform duration-300">
                            Contact
                        </span>
                        <span className="block absolute top-full left-0 group-hover:translate-y-[-100%] transition-transform duration-300 text-emerald-400">
            ...
            <Link to="/patient">
            <button className='mt-5 md:hidden bg-emerald-300 hover:shadow-[0px_0px_30px_14px] shadow-[0px_0px_30px_7px] hover:shadow-cyan-700/50 shadow-green-500/50 text-black px-4 py-2 rounded-full text-sm font-medium hover:bg-slate-100 transition duration-700'>Get Started</button>
            </Link>
